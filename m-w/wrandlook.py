...
# from the Merriam-Webster's Collegiate® Dictionary. API requests are
...
#
# Results are written to files in pre-existing subdir ./cache-coll/
...
# collection of all the dictionary entries for Wordleable words ;-)
#
import json
import os
import random
...
def lookup(word: str):
    ...
    if response:  # Evaluates to True for status codes < 400
        ...
        elif data[0]['meta']['src'] != "collegiate":
...
if __name__ == "__main__":
    ...
    ALL_FILE = "./ALL"
    print(f'Powered by Merriam-Webster\'s Collegiate® Dictionary API', file=sys.stderr, flush=True)
